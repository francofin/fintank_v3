...
def screen(request):
    ...
    return render(request, 'screener/screener.html', data)
...
def screener(request):
    ...
    if request.method == 'POST':
        ...
        output_for_stocks = []
        if ((sector!='') and (size!='')):
            sector_size_screen = json.loads(requests.get(f'https://fmpcloud.io/api/v3/stock-screener?sector='+sector+'&marketCapMoreThan='+size+f'&limit=4000&apikey={fmp_api}').content)
            for item in sector_size_screen:
                output_for_stocks.append(item)
            length = len(output_for_stocks)
        elif sector and size == '':
            sector_screen = json.loads(requests.get(f'https://fmpcloud.io/api/v3/stock-screener?sector='+sector+f'&limit=100&apikey={fmp_api}').content)
            for item in sector_screen:
                output_for_stocks.append(item)
            length = len(output_for_stocks)
        elif size and sector == '':
            if size == "200000000000":
                size_screen = json.loads(requests.get(f'https://fmpcloud.io/api/v3/stock-screener?marketCapMoreThan=180000000000&limit=4000&apikey={fmp_api}').content)
                for item in size_screen:
                    output_for_stocks.append(item)
                length = len(output_for_stocks)
            elif size == "10000000000":
                size_screen = json.loads(requests.get(f'https://fmpcloud.io/api/v3/stock-screener?marketCapMoreThan='+size+f'&marketCapLowerThan=180000000000&limit=4000&apikey={fmp_api}').content)
                for item in size_screen:
                    output_for_stocks.append(item)
                length = len(output_for_stocks)
            elif size == "2000000000":
                size_screen = json.loads(requests.get(f'https://fmpcloud.io/api/v3/stock-screener?marketCapMoreThan='+size+f'&marketCapLowerThan=10000000000&limit=4000&apikey={fmp_api}').content)
                for item in size_screen:
                    output_for_stocks.append(item)
                length = len(output_for_stocks)
            elif size == "300000000":
                size_screen = json.loads(requests.get(f'https://fmpcloud.io/api/v3/stock-screener?marketCapMoreThan='+size+f'&marketCapLowerThan=2000000000&limit=4000&apikey={fmp_api}').content)
                for item in size_screen:
                    output_for_stocks.append(item)
                length = len(output_for_stocks)
            elif size == "50000000":
                size_screen = json.loads(requests.get(f'https://fmpcloud.io/api/v3/stock-screener?marketCapMoreThan='+size+f'&marketCapLowerThan=300000000&limit=4000&apikey={fmp_api}').content)
                for item in size_screen:
                    output_for_stocks.append(item)
                length = len(output_for_stocks)
    ...
    i = 0
    pure_equity_list=[]
    while i < len(output_for_stocks):
        if output_for_stocks[i]['isEtf'] == False:
            pure_equity_list.append(output_for_stocks[i])
        i+=1



    length = len(pure_equity_list)


    paginator = Paginator(pure_equity_list,30)
    ...
    'francois': francois,
    'length': length,
    'pure_equity_list':paged_equities,
...
def stock_item(request, item):
    ...
    ticker_request_raw = item
    stock = ticker_request_raw.upper()
    ...
    return render(request, 'screener/stock.html', data)
...
def stock_search(request):
    ...
    #Read in stock data and get historical chart if avialable from fmp api call
    if request.method =='POST':
        ticker_request_raw = request.POST['ticker']
        stock = ticker_request_raw.upper()
